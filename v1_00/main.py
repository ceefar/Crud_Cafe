...
class Game:
    ...
    def load_data(self):
        ...
        # -- load images -- 
        self.scene_img = pg.image.load(path.join(imgs_folder, SCENE_IMG)).convert_alpha()
        # self.an_img = pg.transform.scale(self.an_img, (140, 140)) # (56, 56))
        # -- load fonts -- 
    ...
    def draw(self):
        ...
        pg.draw.rect(self.screen, DARKGREY, screen_outline_rect, 25) # draws the faux monitor edge around the screen surf # BROWNTONE

        # -- loop sprites in all_sprites group and blit each sprites image to its position - currently unused as trying a slightly different implementation --
