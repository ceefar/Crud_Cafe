    ...
    def update(self):
        ...
        for a_customer in self.customers:
            if isinstance(a_customer, Customer): # ik, it will be duh, but i just want the type hints while testing                
                ...
                # so here we'll do a held down check that will move and also break (as we defo dont wanna do the below check (tho it shouldnt validate on down anyways duh))
                a_customer.move_click_hold_opened_chatbox()
                ...
                if clicked_open_customer:
                    ...
                    if self.display_tabs[0] == "new_orders":   
                        ...
                        break # break the loop on the first rect the mouse collides with as its the one on top and thats the one we want to bring to the front                         
                else:
        ...
        # print(" ")
        # print(f"All Open List => {self.all_open_chat_customers}\n")
    ...
    def draw(self):
        ...
        for index, a_customer in enumerate(self.all_open_chat_customers):
            ...
            if self.display_tabs[0] == "chats":
                if isinstance(a_customer, Customer): 
                    self.chats_tab.image = a_customer.draw_open_chatbox(index, self.chats_tab.image)
        ...
        for sprite in self.all_sprites:
            # -- orders tab --
            # HUGE N0TE > can try use issubclass here, will handle both (this is why i love learning so much)
    ...
    def events(self):
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:
                self.quit()
            # -- mouse down --
            if event.type == pg.MOUSEBUTTONDOWN: # <- own function duh
