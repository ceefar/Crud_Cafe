...
class Customer(pg.sprite.Sprite):
    ...
    # abandoned for now
    def move_click_hold_opened_chatbox(self):
        if self.game.mouse_click_down:
            if self.chatbox_opened_destination_rect:
                if self.chatbox_opened_destination_rect.collidepoint(pg.mouse.get_pos()):
                    self.force_to_mouse_position = True
        else:
            self.force_to_mouse_position = False

        # what you would wanna do is
        # check if force_to_mouse_pos is true
        # if it is ur holding a rect
        # if u click again drop the rect at the mouse pos
        # and reset the var
        # better just to do refactor tho imo
...
# start on da order stuff baybay!
# - section out the areas so main area, right hand bar with order slip, order buttons, and chat windows
# - do this little menu book thing that you can bring up with the item names and numbers, and also ig change page too

# then since no camera, make the new version here with the ui stuff too
# - this way can then use the camera to animate in and out from that position (start end level, and use can too ig - animate the little window with traffic ig, defo make it a city)
# - and then also animate the book coming up easier
# - that being said all of this could still be achieved here, except doing the ui will probably take just as long to add here as to refactor so just refactor lol
...
class Browser_Tab(pg.sprite.Sprite):
    ...
    def draw(self):
        ...
        if isinstance(self, New_Orders_Tab):
            pg.draw.rect(self.image, RED, pg.Rect(20, 20, 20, 20))
        else:
            pg.draw.rect(self.image, BLUE, pg.Rect(40, 40, 20, 20))
